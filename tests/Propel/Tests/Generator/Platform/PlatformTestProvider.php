...
EOF;
    ...
    public static function providerForTestGetUniqueDDL()
    {
        ...
        $column1->getTypeMapping()->copy(new TypeMapping(ColumnType::INTEGER));
        $table->addColumn($column1);
        $column2 = new Column('bar2');
        $column2->getTypeMapping()->copy(new TypeMapping(ColumnType::VARCHAR));
    ...
    public static function providerForTestGetForeignKeysDDL()
    {
        ...
        $column3->getTypeMapping()->copy(new TypeMapping(ColumnType::BLOB));
        $table1->addColumn($column3);
        $table3 = new Table('baz');
        $db->addTable($table3);
        $column4 = new Column('id');
        $column4->getTypeMapping()->copy(new TypeMapping(ColumnType::BINARY));
